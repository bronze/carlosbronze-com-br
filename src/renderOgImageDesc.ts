...
export default async function renderOgImage(
  ...
  const markup = html`<div tw="bg-[#3b4252] flex flex-col w-full h-full">
    <div tw="flex flex-col w-full h-4/5 p-10 justify-center">
      <div tw="mb-6">${title}</div>
      <div tw="flex text-6xl w-full font-bold leading-snug tracking-tight">
        ${description}
      </div>
    </div>
    <div
      ...
      <div tw="flex items-center">
        ...
        <span tw="ml-3 text-zinc-400">carlosbronze.com.br</span>
      </div>
      <div tw="flex items-center">
        <div tw="flex flex-col ml-4">
          <span tw="text-zinc-400">Carlos Bronze</span>
          <span tw="text-blue-400">@carlosbronze</span>
        </div>
      </div>
    </div>
  </div>`
